...
BIN="${REMINDERS_BINARY:-$ROOT/scripts/reminders}"
...
call_probe() {
  ...
  local resolve=()
  [[ -n "$PUBLIC_IP" && "$endpoint" == https://* ]] && resolve=(--resolve-ip "$PUBLIC_IP")
  REMINDERS_MCP_TOKEN="$token" "$PROBE" --transport http --endpoint "$endpoint/mcp" "${resolve[@]}" --tool "$tool" --args "$args" --timeout 45s
}
curl_public() {
  local args=()
  if [[ -n "$PUBLIC_IP" && "$1" == https://* ]]; then
    local host=${1#https://}; host=${host%%/*}
    args=(--resolve "$host:443:$PUBLIC_IP")
  fi
  curl "${args[@]}" "${@:2}" "$1"
}

go build -trimpath -o "$BIN" ./cmd/reminders
go build -trimpath -o "$PROBE" ./cmd/reminders-mcp-probe

# REM.0: real authorized CLI backend, with an explicit negative auth/status control.
if out=$(remindctl status 2>&1) && printf '%s' "$out" | rg -qi 'full access'; then
  record REM.0 PASS "remindctl reports Full access"
else
  record REM.0 FAIL "remindctl not authorized: ${out:-no output}"
fi

# REM.1: real stdio response has bounded pagination metadata.
if out=$("$PROBE" --transport stdio --binary "$BIN" --tool show --args "{\"list\":\"$ALLOWED_LIST\",\"limit\":1}" --timeout 45s 2>&1) && \
...
elif lists=$("$PROBE" --transport stdio --binary "$BIN" --tool lists --timeout 45s 2>&1) && \
   add=$("$PROBE" --transport stdio --binary "$BIN" --tool add --args "{\"title\":\"$title\",\"list\":\"$ALLOWED_LIST\",\"notes\":\"DoD canary; safe to complete\"}" --timeout 45s 2>&1); then
  id=$(printf '%s' "$add" | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d["structuredContent"]["reminder"]["id"])' 2>/dev/null || true)
  if [[ -n "$id" ]] && complete=$("$PROBE" --transport stdio --binary "$BIN" --tool complete --args "{\"id\":\"$id\",\"list\":\"$ALLOWED_LIST\"}" --timeout 45s 2>&1); then
...
# REM.3: missing backend and unauthorized-like failure must be loud.
fake_dir=$(mktemp -d)
cat >"$fake_dir/remindctl" <<'EOF'
#!/usr/bin/env bash
echo 'Reminders access denied; enable Full Access in System Settings' >&2
exit 77
EOF
chmod +x "$fake_dir/remindctl"
if out=$(PATH="$fake_dir:/usr/bin:/bin" "$PROBE" --transport stdio --binary "$BIN" --tool lists --timeout 10s 2>&1); then
  record REM.3 FAIL "denied EventKit fake unexpectedly succeeded"
elif printf '%s' "$out" | rg -qi 'denied|full access|exit status'; then
  record REM.3 PASS "backend denial failed loudly"
else
...
PY
    ...
    local_ok=$("$PROBE" --transport stdio --binary "$BIN" --tool status --timeout 45s 2>/dev/null | rg -q '"authenticated":true' && echo yes || echo no)
    ...
    if [[ "$ALLOW_TUNNEL_RESTART" == 1 && "$TUNNEL_RESTART_HOST" == "$PUBLIC_HOST" && -n "$PUBLIC_HOST" ]] && launchctl print "gui/$(id -u)/com.paulino.icloud-reminders-tunnel" >/dev/null 2>&1; then
      ...
      sleep 1
      if "$PROBE" --transport stdio --binary "$BIN" --tool status --timeout 45s 2>/dev/null | rg -q '"authenticated":true'; then fallback_ok=yes; fi
    ...
    if [[ "$local_ok" == yes && "$remote_ok" == yes && "$fallback_ok" == yes ]] && printf '%s' "$agent" | rg -q 'state = running|pid =' && printf '%s' "$tunnel" | rg -q 'state = running|pid ='; then
      record REM.8 PASS "tunnel stop preserved local MCP; remote backend authenticated; LaunchAgents recovered"
...
doc={"nonce":nonce,"mode":mode,"git_sha":cmd("git","rev-parse","HEAD"),"git_status":cmd("git","status","--short"),
     "environment":{"platform":platform.platform(),"go":cmd("go","version"),"remindctl":cmd("remindctl","--version")},
